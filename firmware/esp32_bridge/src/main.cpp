...
//    * Bumper    — a contact switch (NC) on the front; asserted = forward motion
//                  is vetoed, reverse and turning still pass. NOT latched: the
//                  robot must always be able to back out of what it hit without
//                  waiting for a handshake with the Pi.
//    * Feedback  — measured wheel speeds + battery + temp + safety state are
//                  relayed to the Pi for odometry and health monitoring.
//
...
//    GPIO25                           --- E-stop button, NC to GND, INPUT_PULLUP
//    GPIO26                           --- Bumper switch, NC to GND, INPUT_PULLUP
//
...
static const uint8_t  ESTOP_PIN    = 25;    // NC to GND, INPUT_PULLUP
static const uint8_t  BUMP_PIN     = 26;    // NC to GND, INPUT_PULLUP
static const bool     DEBUG_ENABLE = false; // Serial1 debug (off = clean Pi link)

// Bumper release debounce. Deliberately asymmetric: assert the instant the
// switch reads active (never debounce toward danger — that is milliseconds of
// driving into whatever we just hit), but only release after it has read clear
// for this long, so contact bounce cannot flicker the veto off mid-collision.
static const uint16_t BUMP_CLEAR_MS = 100;

// Which sign of `speed` drives the robot forward. The Pi's mapping in
// hoverboard_bridge makes v > 0 -> speed > 0, so positive is forward.
// ⚠️ VERIFY ON THE BENCH, WHEELS IN THE AIR, before trusting the bumper: if the
// board's mixer turns out inverted, this veto would block reverse and happily
// drive INTO the obstacle — the exact opposite of the intent.
static const bool BUMP_BLOCKS_POSITIVE_SPEED = true;
...
// The uint8 fields are folded into 16-bit words for the checksum, so they come
// in pairs: adding a flag means adding it next to a pad, not on its own.
typedef struct __attribute__((packed)) {
  uint16_t start;
  int16_t  speedL_meas;
  int16_t  speedR_meas;
  ...
  uint8_t  bump;         // 1 = bumper asserted -> forward vetoed (not latched)
  uint8_t  _pad2;
  uint16_t checksum;
} EspFeedback;
...
static bool     bump_asserted     = false;
static uint32_t bump_last_active_ms = 0;
...
static void piSendFeedback() {
  ...
  f.bump        = bump_asserted ? 1 : 0;
  f._pad2       = 0;
  f.checksum    = f.start ^ f.speedL_meas ^ f.speedR_meas ^ f.batVoltage ^
                  f.boardTemp ^ (uint16_t)(f.estop | (f.watchdog_ok << 8)) ^
                  (uint16_t)(f.bump | (f._pad2 << 8));
  Serial.write((uint8_t *)&f, sizeof(f));
}

// ---- Safety -----------------------------------------------------------------
// Bumper. Same fail-safe wiring as the E-stop: NC contact between pin and GND,
// INPUT_PULLUP, so a cut or unplugged bumper wire reads HIGH and vetoes forward
// motion. A broken sensor therefore refuses to drive forward rather than
// silently reporting "all clear" — the failure is visible, not dangerous.
//
// Unlike the E-stop this does NOT latch. A latched bumper would pin the robot
// against whatever it touched until the Pi ran a clear handshake; a plain veto
// lets Nav2 simply back away, and can never make the collision worse.
static void bumpUpdate() {
  const bool active = (digitalRead(BUMP_PIN) == HIGH);
  const uint32_t now = millis();
  if (active) {
    bump_last_active_ms = now;
    if (!bump_asserted) DBG("bump asserted\n");
    bump_asserted = true;               // instant: never debounce toward danger
  } else if (bump_asserted && (now - bump_last_active_ms) >= BUMP_CLEAR_MS) {
    bump_asserted = false;              // release only after a stable-clear window
    DBG("bump cleared\n");
  }
}

// Apply the bumper veto to an outgoing command. Only ever reduces motion:
// forward is zeroed, reverse and steering pass untouched so the robot can
// always escape.
static int16_t applyBumpVeto(int16_t speed) {
  if (!bump_asserted) return speed;
  const bool driving_into_it =
      BUMP_BLOCKS_POSITIVE_SPEED ? (speed > 0) : (speed < 0);
  return driving_into_it ? 0 : speed;
}
...
void setup() {
  ...
  pinMode(BUMP_PIN, INPUT_PULLUP);                    // NC switch to GND
...
void loop() {
  piReceive();
  hoverReceive();
  // Polled every loop, not just at TX time: the switch closes for however long
  // the collision lasts, and sampling it at 50 Hz could miss a glancing hit.
  bumpUpdate();
  ...
  if (now - last_tx_ms >= TX_PERIOD_MS) {
    last_tx_ms = now;
    if (safetyOk()) {
      // The veto is applied to the OUTGOING command only — cmd_speed keeps the
      // Pi's latest wish, so releasing the bumper resumes immediately without
      // waiting for the next frame.
      hoverSend(cmd_steer, applyBumpVeto(cmd_speed));
    } else {
